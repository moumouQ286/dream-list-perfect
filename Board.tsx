    ...
    <div className="flex flex-col h-screen bg-[#F2F2F7]">
                  ...
                  >
                    {isEditing ? (
                      <input
                        ...
                        onBlur={() => { onUpdateTab(tab, editValue); setEditingTab(null); }}
                        onKeyDown={(e) => { if (e.key === 'Enter') { onUpdateTab(tab, editValue); setEditingTab(null); } }}
                      ...
                      <div className="relative">
                        <button
                          ...
                          onDoubleClick={() => { setEditingTab(tab); setEditValue(tab); }}
                          className={cn(
                        ...
                        <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex gap-1 opacity-0 group-hover/tab:opacity-100 transition-opacity z-20 pointer-events-none group-hover/tab:pointer-events-auto">
                          ...
                          <button
                            onClick={(e) => { e.stopPropagation(); onDeleteTab(tab); }}
      ...
      >
        ...
        {!showAllNotes && notes.filter(n => n.ageTab.toString().trim() === currentTab.toString().trim()).length === 0 && (
          <div key={`empty-${currentTab}`} className="absolute inset-0 flex flex-col items-center justify-center p-8 z-10">
            <button
              onClick={onAddNote}
      ...
      <footer className="h-12 bg-white/50 backdrop-blur-sm px-8 flex items-center justify-between text-[11px] text-[#8E8E93] font-medium border-t border-[#D1D1D6] z-50">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={onExport}
            className="flex items-center gap-1 px-3 py-1 rounded-full border bg-white border-[#D1D1D6] text-[#1C1C1E] hover:bg-gray-50 transition-all"
          >
            💾 匯出
          </button>
          <button
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="uppercase tracking-widest">TEST 123</span>
          </div>
        </div>
      </footer>
    </div>
